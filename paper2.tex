...
\begin{abstract}
  ...
  from the MultiDark $N$-body $\Lambda$CDM simulation in a companion paper.} 
  ...
  a physical solution to this apparent contradiction. {\bf Assuming a redshift-independent
  fraction of radio-loud clusters, we demonstrate the unique 
  prospects for low-frequency radio surveys (such as the
  LOFAR Tier~1 survey) to detect about $600\times\rmn{\emph{loud~fraction}[\%]}/10$ 
  hadronically-generated giant and mini radio halos. 
  This shows the potentially of future radio surveys in determining 
...
in order to have appreciable CR transport.} 
...
choices of our CR and magnetic parametrization. In Section~\ref{sec:5}, we show
...
\subsubsection{Halo switch-on/-off mechanism and the radio-X-ray bimodality.}
  ...
  possibility to substantially decrease the radio flux even in merging clusters.} 
...
only in the radio-loud merging NCCC sample, the radio (giant) halos. And (ii) it would be dominant only 
...
This represent just a phenomenological fudge aiming to reproduce what 
...
guidance the results obtained with the surface brightness modeling
on the missing signal fraction. Additionally, in this way, it is tied to the
CR and B properties of a given cluster.  
As $L_{1.4~\rmn{GHz, had}} \sim M_{500}^{1.7}$,
we add an additional mass scaling to reach the
desired $L_{1.4~\rmn{GHz}} \sim M_{500}^4$.
The resulting median (mean) boost is about
32\% (47\%) of the hadronic component.\footnote{The modeling of
...
details and correlation to the hadronic one to future work. 
...
the uncertainty of possible intrinsic spectral features not considered
here (e.g. spectral steepening by CR transport, SZ decrement).}
...
the analytical model for the evolving RLF results in spurious counts due to the 
...
Indeed, a detailed characterization of the instrumental response is needed in order
...
{\bf In particular, the comparison with the predictions of the turbulent reaccelerating 
scenario by \cite{2010A&A...509A..68C,2012arXiv1210.1020C}, where a significantly
smaller number of objects is expected, will eventually help in disentangling different 
...
{\bf We introduce a phenomenological realization of the additional leptonic component in an hybrid scenario,
and show how this can affect the population of high-mass radio-loud NCCCs (radio giant halos), bringing 
its scaling relations in agreement with the the available observational samples. However, due to the nature of 
our approach, i.e., the use of an $N$-body simulation which is volume-limited, only few tens of objects 
