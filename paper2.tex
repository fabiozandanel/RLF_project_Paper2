...
\begin{abstract}
  ...
  explains the radio emission in mini halos (Perseus, Ophiuchus). {\bf By contrast,
  ...
  realized through CR streaming and diffusion (Coma and A2163 at 1.4~GHz). 
  We suggest that a second, leptonic component could be responsible 
  for the missing flux in giant halos within a new hybrid scenario of which 
  we describe possible observational consequences. We then study in
  detail the hadronic component, including an effective parameterization of CR
  transport, using a cosmological complete mock galaxy cluster catalog built 
  from the MultiDark $N$-body $\Lambda$CDM simulation.} 
  Because of CR transport and the different scalings of the
  ...
  a physical solution to this apparent contradiction. {\bf Assuming a plausible fraction
  of radio-loud clusters, fixed at all redshifts, we demonstrate the unique 
  prospects for low-frequency radio surveys (such as the
  LOFAR Tier~1 survey) to detect $\sim600\times\rmn{\emph{loud~fraction}[\%]}/10$ 
  hadronically-generated (giant and mini) radio halos and to probe the underlying 
  physics of radio halos. This shows the potentially of future radio surveys in determining 
  the generation mechanism of halos and the importance of the hadronic component in 
  galaxy clusters, particularly at low masses which are currently unexplored.}
  We make our cosmologically complete multi-frequency mock 
  catalogs for the (non-)thermal cluster emission at different redshifts publicly and freely 
...
%  sources are mainly of leptonic origin. {\bf We use a mock galaxy cluster catalog
%  built from the MultiDark $N$-body $\Lambda$CDM simulation that matches 
%  Sunyaev-Zel'dovich and X-ray scaling relations and luminosity function.} 
%  Because of CR streaming and the different scalings of the
%  X-ray luminosity, $L_{\rmn{X}}$, and the Sunyaev-Zel'dovich flux, $Y$, with
%  gas density, our model can simultaneously reproduce the observed bimodality of
%  radio-loud and radio-quiet clusters at the same $L_{\rmn{X}}$ as well as the
%  unimodal distribution of radio-halo luminosity versus $Y$; thereby suggesting
...
%  to probe the underlying physics of radio halos. We make our cosmologically 
%  complete multi-frequency mock catalogs for the (non-)thermal cluster emission 
%  at different redshifts publicly and freely available on-line
...
luminosity. 
%5Either they harbor a RH or they do not have any detectable diffuse
...
%However, the recent report of spectral flattening with
...
%spectrum \citep{2010MNRAS.409..449P}. 
...
by \cite{2013arXiv1303.4746W} that developed a more rigorous treatment. 
In particular, they show that when considering turbulent
damping, CRs can stream at super-Alfvenic velocities, which is crucial
for CR transport to be of relevant magnitude.} 
...
Recently, \cite{2012MNRAS.421L.112B} presented the first scaling relations
...
microscopic CR transport). {\bf This has been confirmed by \cite{2013arXiv1307.3049S}
and \cite{2013arXiv1306.4379C}.}
...
%significant constraints on the relative CR pressure contribution. 
%The long observational campaign of the Perseus galaxy cluster
...
%2012...VERITAS,2012arXiv1207.6749H}. To summarize, gamma-ray observations are 
%not yet sensitive enough to severely challenge hadronic models of RHs.
...
{\bf The main scope of this work is to improve the classical hadronic model with
the inclusion of possible CR transport phenomena, trying in this way to solve
its main issues when comparing to observations and to make predictions for
...
extended hadronic model to a cosmologically complete mock galaxy cluster catalog 
built from the MultiDark $N$-body $\Lambda$CDM simulation
in Zandanel, Pfrommer \& Prada (2013; hereafter Paper~I). 
...
constraints and provide predictions for the hadronically-generated
...
distribution (assuming isotropy in momentum space). {\bf We are interested 
in calculating the radio synchrotron and gamma-ray emission resulting from 
secondaries of hadronic CR interactions with protons of the ICM (see
Appendix~\ref{app:A} and Appendix~\ref{app:C} for the details).} 
...
which implies an inverse scaling with the CR number density \citep{2013arXiv1303.4746W}.
Once CR streaming depletes the CR number density, this causes a
...
which however did not account for CR transport. {\bf To include the latter, we adapt 
the analytical formalism from \citet{2011A&A...527A..99E}. This yields a model that includes 
the necessary CR transport physics and is able to predict radio and gamma-ray 
emission. Note that this approach is not fully self-consistent and points to
...
\citet{2011A&A...527A..99E}, which uses a $\beta$-profile gas parameterizatio, 
in order to account for different kind of ICM gas profiles, such as our generalized 
Navarro-Frank-White (GNFW) profiles derived in Paper~I from X-ray observations.} 
...
Appendix~\ref{app:A}). 
%This extension has to be applied to $A_{\nu}$, to the factor $(\epsilon_{\rmn{B}}(R)/
%\epsilon_{B_{\rmn{c}}})^{(\alpha-2)/4}$ of equation~(\ref{eq:jnu}), and to
%equation~(\ref{eq:Csimple_1}). 
%While the modifications are straight forward in
...
%By scanning the allowed parameter space and asserting Bayesian priors that rely on
...
%conclusions on the applicability of the hadronic model for RHs.  
...
to the measured RH flux. {\bf Additionally, we want to stress once more that
CR streaming can also cause spectral steepening \citep{2013arXiv1303.4746W}.}  
%This demonstrates that the magnitude of the SZ effect
...
scope of this work.}  
%Interestingly, as shown in Fig.~\ref{fig:SB_Coma}, the 1.4
...
distributions in turbulent clusters. However, this does not help in, e.g.,
...
neglected other (leptonic) emission components, such as re-accelerated electrons. 
...
(i) Merging clusters are not spherically symmetric as can be seen in Coma and
  ...
  respectively. 
  %While this approach is well suited to describe large-scale
...
The small cluster sample analysed here is only meant to serve as a proof
...
the parameter space that the physics modeled here demands is quite large
encompassing also $\gamma_{\rmn{tu}}$ and the CR acceleration efficiency
...
%which 
%we will then scrutinize statistically in the following Sections, namely by investigating the
...
%While we leave the origin of the leptonic component unspecified, 
There are a number of plausible processes for the latter. 
These includes turbulent reacceleration of primary or secondary (hadronic)
...
%feedback or by supernova-driven galactic winds. 
...
\subsubsection{Halo switch-on/-off mechanism and the radio-X-ray bimodality.}
  ...
  {\bf Additionally, \cite{2013arXiv1303.4746W} opened the possibility
  to substantially decrease the radio flux even in merging clusters as
  they found that turbulence seems to promote outward streaming 
  more than inward advection.} This should
...
the information on its mass, radius and phenomenologically assigned 
...
($R_{500}/2$) {\bf as suggested by current gamma-ray limits}. 
...
\caption{Radio-to-X-ray and radio-to-SZ scaling relations in our extended CR model
  ...
  {\bf relation from the PSZ(V) sample (direct integration method) 
...
a median redshift of $z \approx XXX$, which compares favourably with our MultiDark 
$z = 0.2$ snapshot. The same comments regarding the small sample size of RHs, selection 
biases, and systematic uncertainties in the luminosity measurements also apply here.}
...
%As a consequence, the highly peaked gas profiles
%of CCCs have less impact on $Y_{\rmn{SZ}}$ than on $L_{\rmn{X,bol}}$. 
...
that would steepen the slope of our model scaling relation at high masses.
...
of an increasingly important leptonic component at high masses.} 

{\bf Therefore, we attempt to model such a leptonic emission component in an 
hybrid scenario. We do not expect this component to alter significantly our conclusions
regarding the luminosity functions of next sections for two main reasons. (i) It would be present only in 
the radio-loud merging NCCC sample. And (ii) it would be dominant only at very high masses 
because the dissipated energy that is available for energizing fossil electrons should be a fraction 
of the thermal energy which scales with cluster mass as $E_\rmn{th}\propto M_{200}^{5/3}$ \citep[][for 
magneto-turbulent reacceleration models]{2005MNRAS.357.1313C}. In fact, in the $z = 0.2$ snapshot 
...
As an example of a possible leptonic component, we boost the total flux of these 31 radio-loud NCCCs, according to 
$L_{1.4~\rmn{GHz, boosted}}= L_{1.4~\rmn{GHz}}\times(1+(M_{200}/10^{15}~h_{70}^{-1}$~M$_{\odot})^{5/3})$,
from about 30\% to about 200\% for the most massive one, with a
median (mean) boost of about 46\% (58\%).\footnote{The modeling of
   ...
   the required fraction of leptionic component could be smaller.} 
The resulting boosted population is shown in both panels of Fig.~\ref{fig:PLSZ}. The 
...
observations. {\bf Additionally, we do not include the possible leptonic emission 
component in the analysis of next sections, deferring the study of its physical 
details and correlation to the hadronic one to future work. Due to the nature
of our approach, the mock galaxy cluster sample is affected by incomlpetness 
in the highest-mass range because of the limited volume of the MultiDark simulation 
(Paper~1). Only a small number of objects, which lie in this mass range, would be affected 
by such a correction, as shown above and in Fig.~\ref{fig:PLSZ}. These are not statistically 
significant compared to the numbers we are going to discuss. Therefore, we prefer to point the reader to the turbulent 
reacceleration model predictions by \cite{2010A&A...509A..68C, 2012arXiv1210.1020C} for comparison, 
and to get a flavour of how an additional leptonic component would change our predictions.} 
%Note that for a given set of parameter choices, our RH fluxes are conservative lower 
%limits; in particular for the subsample of radio loud high-mass NCCCs.
...
{\bf Note that the low-frequency predictions of our model suffer from 
the uncertainty of possible intrinsic spectral features that may differ
from our assumed power-law.}
%Note that calibrating our model to 1.4~GHz observations may cause an over- or
...
the analytical model for the evolving RLF suffers due to the non-perfect matching 
with simulations in this regime, as explained above.}
...
elucidate the critical impact of the detection threshold on the number observable RHs.
Indeed, a detailed characterisation of the instrumental response is needed in order
to obtain more precise estimations.}

{\bf The LOFAR Tier~1 survey at 120~MHz should be able to detect about
$600$ clusters hosting RHs, considering the hadronic
component only (see \citealp{2010A&A...509A..68C,2012arXiv1210.1020C}
for predictions for giant halos in the turbulent reacceleration scenario)}. The precise number 
of detections depends strongly on the underlying assumptions. There are two main uncertainties
...
{\bf In particular, the comparison with the predictions of, e.g., the turbulent reaccelerating 
scenario by \cite{2010A&A...509A..68C,2012arXiv1210.1020C}, where a significantly
smaller number of RHs is expected, will eventually help in disentangling different 
models and in identifying the importance of the hadronic component in galaxy clusters.}
...
$N$-body simulation in a companion paper (Paper~1).} We select a representative 
...
{\bf We show how a possible leptonic component in an hybrid scenario could affect 
the population of high-mass radio-loud NCCCs, bringing its scaling relations in agreement 
with the the available observational samples. However, due to the nature of our approach, 
i.e., the use of an $N$-body simulation which is volume-limited, only few tens of objects 
would be affected by such a correction. These are not statistically significant against our 
whole sample and, therefore, we refrain from adding this component in the analysis
of the luminosity functions and in making predictions for future surveys.}

Assuming a {\bf constant} fraction of 10\% radio-loud clusters, we demonstrate that our 
...
To summarize, we have constructed a model for the ICM (in Paper~I) and CR distributions 
...
Those contain the quantities $\rho_{\rmn{gas}}$, $L_{\rmn{X, bol}}$, $Y_{\rmn{X}}$, $Y_{\rmn{SZ}}$,
$L_{120~\rmn{MHz}}$, $L_{1.4~\rmn{GHz}}$ and $L_{\gamma}$ (among many others). We hope 
that the community can make valuable use of these catalogues in
...
and Shea Brown for a reanalysis of their 352~MHz radio map of Coma, and for useful discussions. 
We thank Matteo Murgia for kindly providing the radio surface brightness profiles of Ophiuchucs
and Abell~2163 that have been recomputed with respect to the Reiprich \& B\"{o}ringher (2002) cluster 
positions, and Wolfgang Reich for providing the 1.4~GHz radio map of Coma.  
We also thank Harald Ebeling, Stefan Gottl{\"o}berg, Anatoly Klypin, Andrey Kravtsov, Adam Mantz, 
Frazer Pearce, Miguel-Angel Perez Torres, Huub R{\"o}ttgering, Jos\'e Alberto Rubi\~no, and Gustavo 
Yepes, for the useful discussions and advices.  Finally, we thank the MultiDark database
...
indicator of the large uncertainty in the RLF. {\bf Note that, as explained in 
Section~\ref{sec:5}, we refrain from use the newly published results of \cite{2013arXiv1306.3102K} 
